...
import { subscribeEvolution } from './services/evolve';
    ...
    <div className="v2-agent-scrim" onMouseDown={close}>
      <div className="v2-agent-modal" onMouseDown={(e) => e.stopPropagation()}>
        ...
        <div className="v2-agent-modal-bd">
          ...
          {/* 自进化记忆/技能改在「会话输入框底部的实时条」(AgentEvolveStrip) 里查看与管理。 */}
        </div>
...
/** 进化提示：监听后台反思结果，弹一条「@xx 进化了：…」轻提示（潜移默化、不打断）。 */
const EvolutionNotifier: React.FC = () => {
  useEffect(() => subscribeEvolution((d) => {
    const agent = listAgents().find((a) => a.id === d.agentId);
    const who = agent ? `@${agent.name}` : 'Agent';
    if (d.event?.summary) toast(`${who} 进化了：${d.event.summary}`);
  }), []);
  return null;
};

/** 单例挂载点：升格/编辑弹层 + 进化提示。Agent 清单已迁到右侧 Agent 面板。 */
export const AgentsManagerHost: React.FC = () => (<><AgentPromoteModal /><EvolutionNotifier /></>);

...
import { getSecret, SECRET_KEYS } from '../services/configStore';
import { reflectTurn, writeMemoryFile as evolveWriteMemoryFile } from './services/evolve';

/** 升格 agent 的系统提示注入 = 人设 + 「被引用的记忆文件」指引。普通会话返回 undefined。
 *  顺带确保 .chaya/AGENT_MEMORY.md 已生成（首次召唤时即便没沉淀也写个占位）。 */
const AGENT_MEMORY_POINTER =
  '你是一个拥有长期记忆的本地 Agent。开始任务前，请先阅读当前工作目录下的 `./.chaya/AGENT_MEMORY.md`' +
  '（若存在）：其中记录了用户的长期偏好与你已掌握的方法 / SOP，请据此保持一致、复用既有做法。';
function buildAgentAppend(sid: string | null | undefined, dir: string): string | undefined {
  if (!sid) return undefined;
  const agent = agentBySession(sid);
  if (!agent) return undefined;
  void evolveWriteMemoryFile(agent.id, dir);
  return [agent.systemPrompt ? agent.systemPrompt.trim() : '', AGENT_MEMORY_POINTER].filter(Boolean).join('\n\n');
}

/** 非 claude provider 的等价物：无 systemPrompt 通道 → 返回前缀进 prompt（人设 + 记忆文件指引）。
 *  同样顺带确保记忆文件已生成。普通会话返回 ''。 */
function buildAgentPromptPrefix(sid: string | null | undefined, dir: string): string {
  if (!sid) return '';
  const agent = agentBySession(sid);
  if (!agent) return '';
  void evolveWriteMemoryFile(agent.id, dir);
...
export function useLocalAgent(active: boolean, provider: ProviderId, typewriter: TypewriterConfig = DEFAULT_TYPEWRITER) {
  ...
  // 优化 B：升格 agent 反思去抖——回合密集时只在「静默 ~8s 且无新回合在跑」时蒸馏一次。
  const reflectTimers = useRef<Map<string, number>>(new Map());
  ...
  // cursor headless 必需的 API Key（本地 SQLite 凭证库；随 send/warm 注入主进程）。
  ...
  const fetchCursorKey = useCallback(async (): Promise<string | null> => {
    ...
    cursorKeyRef.current = getSecret(SECRET_KEYS.cursorApiKey) || null;
  ...
  const openSession = useCallback(async (projPath: string, sid: string, title: string, sourceCwd?: string) => {
    ...
    if (det?.live) void localAgent.warm({ provider: prov, cwd: dir, lane: paneLane(paneKey), sessionId: sid, permMode: pm, model: md, reasoning: rs, mcp: mc, apiKey: prov === 'cursor' ? cursorKeyRef.current : undefined, appendSystemPrompt: prov === 'claude' ? buildAgentAppend(sid, realDir(paneKey)) : undefined });
  ...
  const ensureAgentSessionTab = useCallback((projPath: string, sid: string, title: string, prov: ProviderId, permMode: PermMode): string => {
    ...
    if (det?.live) void localAgent.warm({ provider: prov, cwd: dir, lane: paneLane(paneKey), sessionId: sid, permMode, model: modelMemRef.current[sid] || undefined, mcp: mcpMemRef.current[sid] || undefined, apiKey: prov === 'cursor' ? cursorKeyRef.current : undefined, appendSystemPrompt: prov === 'claude' ? buildAgentAppend(sid, dir) : undefined });
  ...
  /** 升格 agent 会话专属：回合收尾时后台反思（蒸馏偏好/笔记/技能 + 负反馈闭环）。
   *  best-effort，fire-and-forget；普通会话 / 异常回合 / 空答复 → 跳过。 */
  function maybeReflectTurn(cwd: string, errStatus: string) {
    if (errStatus) return;                                  // 异常回合不沉淀
    const tab = tabsRef.current.find((x) => x.cwd === cwd);
    if (!tab) return;
    const sid = pendingByCwd.current.get(cwd) || tab.sessionId;
    if (!sid) return;
    const agent = agentBySession(sid);
    if (!agent) return;                                     // 只对升格 agent 会话进化
    const textOf = (m: TranscriptMessage) =>
      m.parts.filter((p) => p.kind === 'text').map((p) => (p as { kind: 'text'; text: string }).text).join('').trim();
    const response = tab.liveMsgs.filter((m) => m.role === 'assistant').map(textOf).join('\n').trim();
    if (response.length < 24) return;                       // 答复太短不值得蒸馏
    let task = '';
    for (let i = tab.messages.length - 1; i >= 0; i--) {
      if (tab.messages[i].role === 'user') { task = textOf(tab.messages[i]); break; }
    }
    if (!task) return;
    const payload = {
      agentId: agent.id,
      ...
      cwd: realDir(cwd),
      model: modelMemRef.current[sid],
      mcp: mcpMemRef.current[sid],
      task, response,
    };
    // 去抖：清掉该 cwd 上一个待跑的反思，重排到最新这次（连发只蒸馏最后一轮）。
    const prev = reflectTimers.current.get(cwd);
    if (prev) window.clearTimeout(prev);
    const fire = () => {
      reflectTimers.current.delete(cwd);
      const t = tabsRef.current.find((x) => x.cwd === cwd);
      if (t?.running) {   // 又有新回合在跑 → 推迟,等它静下来再蒸馏(不和主对话抢 CLI/配额)
        reflectTimers.current.set(cwd, window.setTimeout(fire, 5000));
        return;
      }
      void reflectTurn(payload);
    };
    reflectTimers.current.set(cwd, window.setTimeout(fire, 8000));
  }

  function finalizeTurn(cwd: string, errStatus: string) {
    // 升格 agent 会话：抢在 liveMsgs 被清空前抓本回合的「问/答」，回合收尾后后台反思（自进化）。
    maybeReflectTurn(cwd, errStatus);
    const sm = smoothRef.current.get(cwd);
  ...
  const dispatchTurn = useCallback(async (cwd: string, sendText: string, displayText: string, attachments: Attachment[], clearComposer: boolean, skillName?: string) => {
    ...
    // agent 人设 + 长期记忆指引：
    //  · claude 走真·系统提示注入（buildAgentAppend，每轮在、直接对话也生效）；
    //  · 其它 provider 无 systemPrompt append 通道 → 把简短的记忆文件指引前缀到「发给 CLI 的 prompt」
    //    （气泡仍显示用户原话，sendText/displayText 本就解耦）。
    let outText = sendText;
    let sysAppend: string | undefined;
    if (prov === 'claude') {
      sysAppend = buildAgentAppend(sid, realDir(cwd));
    } else {
      const prefix = buildAgentPromptPrefix(sid, realDir(cwd));
      if (prefix) outText = `${prefix}\n\n${sendText}`;
    }
    const res = await localAgent.send({ provider: prov, cwd: realDir(cwd), lane: paneLane(cwd), sessionId: sid, prompt: outText, permMode: tab.permMode, model: tab.model, reasoning: tab.reasoning, mcp: tab.mcp, apiKey, attachments, appendSystemPrompt: sysAppend });

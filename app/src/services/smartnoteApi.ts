 ...
 * Docs / backend project: ~/aiproj/smartnote
 *
 * Auth model:
...
import {
  getConfig, setConfig, delConfig, getSecret, setSecret, delSecret, getSecretMeta,
...
/* ============================================================
   Config — base URL + API key stored in local SQLite (configStore)
   ============================================================ */

export function getSmartnoteBaseUrl(): string {
  const v = getConfig(CONFIG_KEYS.smartnoteBase);
  if (v && v.trim()) return v.trim().replace(/\/$/, '');
  return DEFAULT_BASE;
}
export function setSmartnoteBaseUrl(url: string): void {
  const t = (url || '').trim().replace(/\/$/, '');
  if (t) setConfig(CONFIG_KEYS.smartnoteBase, t);
  else delConfig(CONFIG_KEYS.smartnoteBase);
}
export function getSmartnoteApiKey(): string {
  return getSecret(SECRET_KEYS.smartnoteApiKey);
}
export function setSmartnoteApiKey(k: string): void {
  if (k) setSecret(SECRET_KEYS.smartnoteApiKey, k);
  else delSecret(SECRET_KEYS.smartnoteApiKey);
  // Clear cached JWT so next call re-exchanges
  delSecret(SECRET_KEYS.smartnoteJwt);
}
...
async function fetchJwt(): Promise<string> {
  ...
  setSecret(SECRET_KEYS.smartnoteJwt, data.jwt, { exp: data.expires_at });
  return data.jwt;
}

async function ensureJwt(): Promise<string> {
  const now = Math.floor(Date.now() / 1000);
  const jwt = getSecret(SECRET_KEYS.smartnoteJwt);
  const exp = Number(getSecretMeta(SECRET_KEYS.smartnoteJwt).exp || 0);

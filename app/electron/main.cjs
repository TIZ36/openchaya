...
const { registerConfig } = require('./configDb.cjs');
const { registerLocalAgent, killAllSessions } = require('./localAgent.cjs');
const { registerEvolve } = require('./evolve.cjs');
...
// 本地 SQLite 配置库：用户 API 信息 / SmartNote / cursor / MCP OAuth 凭证的唯一真源。
// DB 懒打开（首个请求时，此时 app 已 ready），仅在此注册 IPC handler。
registerConfig(ipcMain, app);
// 本地 Agent 桥：纯本地功能，与后端无关。仅注册一次。
registerLocalAgent(ipcMain, dialog);
// 自进化引擎：升格 agent 会话 post-turn 反思 + 成熟阶梯，落同一个 chaya.db。
registerEvolve(ipcMain, app);
